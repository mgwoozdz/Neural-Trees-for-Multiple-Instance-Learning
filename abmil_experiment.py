...
# determine indexes of train+val and test splits
train_val_idxs, test_idxs, train_val_labels, test_labels = train_test_split(np.arange(len(plain_dataset)),
...
for nth_split, (train_idxs, val_idxs) in enumerate(skf.split(train_val_idxs, train_val_labels)):
    ...
    train_loader = DataLoader(Subset(augmented_dataset, train_val_idxs[train_idxs]), batch_size=1, shuffle=True)
    valid_loader = DataLoader(Subset(plain_dataset, train_val_idxs[val_idxs]), batch_size=1, shuffle=False)
