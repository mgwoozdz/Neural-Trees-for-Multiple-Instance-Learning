...
class MnistBags(data_utils.Dataset):
    def __init__(self, target_number=9, mean_bag_length=10, var_bag_length=1, num_bag=1000, seed=7, train=True):
        ...
        self.bags, self.labels = self._form_bags()

    def _form_bags(self):
        if self.train:
            train_loader = data_utils.DataLoader(datasets.MNIST('../datasets/data',
                                                                train=True,
                                                                download=True,
            ...
            while valid_bags_counter < self.num_bag:
                ...
                if (self.target_number in labels_in_bag) and (label_of_last_bag == 0):
                    ...
                    index_list = list(map(lambda x: x.item(), index_list))
            ...
            test_loader = data_utils.DataLoader(datasets.MNIST('../datasets/data',
                                                               train=False,
                                                               download=True,
                                                               transform=transforms.Compose([
                                                                    transforms.ToTensor(),
